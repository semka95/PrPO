...
def main():
    ...
    # t1 = timeit.Timer(functools.partial(gen_str, args.data_num, args.charset, args.length))
    # t2 = timeit.Timer(functools.partial(gen_str_mod, args.data_num, args.charset, args.length))
    # print(t1.timeit(1))
    # print(t2.timeit(1))
    return
...
# def gen_str_mod(data_num, charset, length):
#     strings = [''.join(random.choice(charset) for _ in range(length)) for _ in range(data_num)]
#     return '\n'.join(strings)
